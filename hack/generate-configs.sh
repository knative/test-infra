...
go run "${CONFIG_GENERATOR_DIR}" \
    ...
    --prow-jobs-config-output="${REPO_ROOT_DIR}/prow/jobs/config.yaml" \
    --testgrid-config-output="${REPO_ROOT_DIR}/config/prow/testgrid/testgrid.yaml" \
    --k8s-testgrid-config-output="${REPO_ROOT_DIR}/config/prow/k8s-testgrid/k8s-testgrid.yaml" \
    "${REPO_ROOT_DIR}/prow/config_knative.yaml"
