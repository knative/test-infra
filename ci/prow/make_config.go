...
const (
	...
	flakyTestReporterPeriodicJobCron = "0 12 * * *" // Run at 4:00PST/5:00PST every day (12:00 UTC)
...
var (
	...
	flakyTestReporterDockerImage string
...
`
	...
	// periodicJob is the template for periodicJob jobs.
	periodicJob = `
- cron: "[[.CronString]]"
...
`
	...
	// flakyTestReporterPeriodicJob is the template for the flaky tests reporter job.
	flakyTestReporterPeriodicJob = `
- cron: "[[.CronString]]"
  name: [[.PeriodicJobName]]
  agent: kubernetes
  decorate: true
...
// generateFlakyTestReporterPeriodicJob generates the flaky tests reporter job config.
func generateFlakyTestReporterPeriodicJob() {
	var data periodicJobTemplateData
	data.Base = newbaseProwJobTemplateData("knative/test-infra")
	data.PeriodicJobName = "ci-knative-flakytestreporter"
	data.CronString = flakyTestReporterPeriodicJobCron
	data.Base.Image = flakyTestReporterDockerImage
	addExtraEnvVarsToJob(&data.Base)
	configureServiceAccountForJob(&data.Base)
	addVolumeToJob(&data.Base, "/etc/github-token", "github-token", true)
	addVolumeToJob(&data.Base, "/etc/slack-token", "slack-token", true)
	executeJobTemplate("periodic flaky tests reporter", flakyTestReporterPeriodicJob, "presubmits", "", data.PeriodicJobName, false, data)
}
...
func main() {
	...
	flag.StringVar(&flakyTestReporterDockerImage, "flaky-test-reporter-docker", "gcr.io/knative-tests/test-infra/flaky-test-reporter:latest", "Docker image for flaky test reporting tool")
	...
	generateFlakyTestReporterPeriodicJob()
