...
func NewSubscriberClient(ctx context.Context, projectID string, subName string) (*SubscriberClient, error) {
	...
	return &SubscriberClient{sub}, err
}
...
func validateSubscription(ctx context.Context, sub SubscriberOperation) error {
	if exists, err := sub.Exists(ctx); err != nil {
		return err
	} else if !exists {
		return fmt.Errorf("the subscription (%s) does not exist", sub.String())
	}

	log.Printf("Subscription %v is found\n", sub)
	return nil
}
