...
func createHistoryUnicode(rd *RepoData, comment, testFullName string) string {
	...
	if len(oldHistory) >= 2 {
		...
		if len(oldHistoryEntries) > 9 {
			oldHistoryEntries = oldHistoryEntries[:9]
		}
		res = fmt.Sprintf("\n%s%s", currentUnicode, strings.Join(oldHistoryEntries, "\n"))
	} else {
		res = fmt.Sprintf("\n%s\n", currentUnicode)
	}
...
func updateIssue(fi *flakyIssue, newComment string, ts *TestStat, dryrun *bool) error {
	...
	if len(latestStatus) >= 2{
		switch latestStatus[1] {
			case passedStatus:
				passedLastTime = true
			case flakyStatus, failedStatus, lackDataStutus:
...
func findExistingComment(issue *github.Issue, issueIdentity string) (*github.IssueComment, error) {
	...
	for i, comment := range comments {
		...
		if testNameFromComment := regexp.MustCompile(reTestIdentifier).FindStringSubmatch(*comment.Body);
			len(testNameFromComment) >= 2 && issueIdentity == testNameFromComment[1] {
...
func getFlakyIssues() (map[string][]*flakyIssue, error) {
	...
	for k, v := range issuesMap {
		...
		fmt.Sprintln(k, hasOpen, hasClosed)
