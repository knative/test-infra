...
func checkPoolCap(cp *clerk.ClusterParams) {
	...
	difference := DefaultOverProvision - numAvail - numWIP
	if difference > 0 {
		// create cluster if not meeting overprovisioning criteria
		for i := int64(0); i < difference; i++ {
			CreateCluster(cp)
		}
	}
}

func GetGKEClusterConfig(c *clerk.Cluster) *kubetest2.GKEClusterConfig {
	return &kubetest2.GKEClusterConfig{}
}
