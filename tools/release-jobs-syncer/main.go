...
func main() {
	...
	regenConfigScript := flag.String("regen-config-script",
		filepath.Join(helpers.MustGetRootDir(), "hack/generate-configs.sh"), "Path of the script for regenerating config")
	...
	if err := pkg.UpdateReleaseBranchConfig(gc, *prowJobConfigRootPath, *regenConfigScript); err != nil {
